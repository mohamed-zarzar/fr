...
import { ManagerSelect } from '@/components/selects/ManagerSelect';
...
export default function ManagerAttendance() {
  ...
  const { data: absencesRes, isLoading: absLoading } = useQuery({ queryKey: ['manager-absences', absFilter], queryFn: () => managerAttendanceApi.getAbsences(absFilter) });
  const { data: latesRes, isLoading: lateLoading } = useQuery({ queryKey: ['manager-lates', lateFilter], queryFn: () => managerAttendanceApi.getLates(lateFilter) });
  ...
  const bulkAbsStatusMut = useMutation({ mutationFn: ({ ids, status }: { ids: string[]; status: any }) => managerAttendanceApi.bulkUpdateAbsenceStatus(ids, status), onSuccess: () => { invalidate(); absBulk.clear(); toast.success(t('common.statusUpdated')); } });
  const bulkAbsDeleteMut = useMutation({ mutationFn: (ids: string[]) => managerAttendanceApi.bulkDeleteAbsences(ids), onSuccess: () => { invalidate(); absBulk.clear(); toast.success(t('common.bulkDeleted')); } });
  const bulkLateStatusMut = useMutation({ mutationFn: ({ ids, status }: { ids: string[]; status: any }) => managerAttendanceApi.bulkUpdateLateStatus(ids, status), onSuccess: () => { invalidate(); lateBulk.clear(); toast.success(t('common.statusUpdated')); } });
  const bulkLateDeleteMut = useMutation({ mutationFn: (ids: string[]) => managerAttendanceApi.bulkDeleteLates(ids), onSuccess: () => { invalidate(); lateBulk.clear(); toast.success(t('common.bulkDeleted')); } });
  ...
  const getManagerName = (a: ManagerAbsence | ManagerLate) =>
    a.firstName ? `${a.firstName} ${a.lastName ?? ''}`.trim() : a.managerId;
  ...
  const handleImport = async (file: File) => {
    const res = await managerAttendanceApi.importExcel(file);
  ...
  const generateBulkRows = () => {
    ...
    setBulkRows(Array.from({ length: count }, () => ({ managerId: '', date: today(), isJustified: false, period: 10 })));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <FilterBar showClear={!!(filter.entityId || filter.dateFrom || filter.dateTo || filter.levelId || filter.classId)} onClear={() => { setFilter({}); setAbsPage(1); setLatePage(1); }}>
        <div className="space-y-1">
          ...
          <ManagerSelect
            ...
            allOption={{ value: 'all', label: t('common.allManagers') }}
            className="w-44"
          />
        </div>
        {activeView !== 'calendar' && (
      ...
      <Tabs value={tab} onValueChange={v => setTab(v as any)}>
        ...
        <TabsContent value="absences" className="space-y-4">
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <Button size="sm" variant="outline" onClick={() => managerAttendanceApi.exportExcel(filter)}><Download className="me-2 h-4 w-4" />{t('common.export')}</Button>
          </div>
          ...
          {absLoading ? <Skeleton className="h-48 w-full" /> : absView === 'calendar' ? (
            <AttendanceCalendarView items={absences} type="absences" getEntityName={(item) => getManagerName(item as ManagerAbsence)} onEdit={(item) => { resetAbsForm(item as any); setEditingAbsence(item as any); }} onDelete={(item) => setDeleteTarget({ id: item.id, type: 'absence' })} />
          ) : (
              ...
              <div className="rounded-md border overflow-auto">
                <Table>
                  <TableHeader><TableRow>
                    ...
                    <TableHead>{t('common.manager')}</TableHead><TableHead>{t('common.date')}</TableHead><TableHead>{t('common.status')}</TableHead><TableHead>{t('common.justified')}</TableHead><TableHead>{t('common.reason')}</TableHead><TableHead className="w-24">{t('common.actions')}</TableHead>
                  </TableRow></TableHeader>
                  <TableBody>
                    {absences.length === 0 ? <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-8">{t('attendance.noAbsences')}</TableCell></TableRow> :
                      absences.map(a => (
                        <TableRow key={a.id}>
                          <TableCell><Checkbox checked={absBulk.selectedIds.has(a.id)} onCheckedChange={() => absBulk.toggle(a.id)} /></TableCell>
                          <TableCell>{getManagerName(a)}</TableCell>
        ...
        <TabsContent value="lates" className="space-y-4">
          ...
          {lateLoading ? <Skeleton className="h-48 w-full" /> : lateView === 'calendar' ? (
            <AttendanceCalendarView items={lates} type="lates" getEntityName={(item) => getManagerName(item as ManagerLate)} onEdit={(item) => { resetLateForm(item as any); setEditingLate(item as any); }} onDelete={(item) => setDeleteTarget({ id: item.id, type: 'late' })} />
          ) : (
            <div className="space-y-3">
              <div className="rounded-md border overflow-auto">
                <Table>
                  <TableHeader><TableRow>
                    ...
                    <TableHead>{t('common.manager')}</TableHead><TableHead>{t('common.date')}</TableHead><TableHead>{t('common.period')}</TableHead><TableHead>{t('common.status')}</TableHead><TableHead>{t('common.justified')}</TableHead><TableHead>{t('common.reason')}</TableHead><TableHead className="w-24">{t('common.actions')}</TableHead>
                  </TableRow></TableHeader>
                  <TableBody>
                    {lates.length === 0 ? <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground py-8">{t('attendance.noLates')}</TableCell></TableRow> :
                      lates.map(l => (
                        <TableRow key={l.id}>
                          <TableCell><Checkbox checked={lateBulk.selectedIds.has(l.id)} onCheckedChange={() => lateBulk.toggle(l.id)} /></TableCell>
                          <TableCell>{getManagerName(l)}</TableCell>
      ...
      <Dialog open={absDialog || !!editingAbsence} onOpenChange={o => { if (!o) { setAbsDialog(false); setEditingAbsence(null); } }}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2"><Label>{t('common.manager')}</Label><ManagerSelect value={formManagerId} onChange={setFormManagerId} requiredResult={formManagerId ? [formManagerId] : []} className="w-full" /></div>
      ...
      <Dialog open={bulkAbsDialog} onOpenChange={setBulkAbsDialog}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            {bulkRows.map((row, idx) => (
              <div key={idx} className="flex gap-2 items-end flex-wrap border-b border-border pb-2">
                <div className="flex-1 min-w-[180px] space-y-1"><Label className="text-xs">{t('common.manager')}</Label><ManagerSelect value={row.managerId} onChange={v => updateBulkRow(idx, 'managerId', v)} requiredResult={row.managerId ? [row.managerId] : []} disabledIds={bulkRows.filter((_, i) => i !== idx).map(r => r.managerId).filter(Boolean)} className="w-full" /></div>
          ...
          <DialogFooter><Button variant="outline" onClick={() => setBulkAbsDialog(false)}>{t('common.cancel')}</Button><Button onClick={() => { const valid = bulkRows.filter(r => r.managerId); if (!valid.length) { toast.error(t('attendance.addValidRow')); return; } const ids = valid.map(r => r.managerId); if (new Set(ids).size !== ids.length) { toast.error(t('attendance.duplicateEntity')); return; } bulkAbsMut.mutate(valid.map(r => ({ managerId: r.managerId, date: bulkShared ? bulkSharedDate : r.date, isJustified: r.isJustified, reason: r.isJustified ? r.reason : undefined }))); }}>{t('attendance.addCountAbsences', { count: bulkRows.filter(r => r.managerId).length })}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={bulkLateDialog} onOpenChange={setBulkLateDialog}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          <DialogFooter><Button variant="outline" onClick={() => setBulkLateDialog(false)}>{t('common.cancel')}</Button><Button onClick={() => { const valid = bulkRows.filter(r => r.managerId); if (!valid.length) { toast.error(t('attendance.addValidRow')); return; } const ids = valid.map(r => r.managerId); if (new Set(ids).size !== ids.length) { toast.error(t('attendance.duplicateEntity')); return; } bulkLateMut.mutate(valid.map(r => ({ managerId: r.managerId, date: bulkShared ? bulkSharedDate : r.date, isJustified: r.isJustified, reason: r.isJustified ? r.reason : undefined, period: r.period ?? 10 }))); }}>{t('attendance.addCountLates', { count: bulkRows.filter(r => r.managerId).length })}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <ExcelImportDialog open={importAbsOpen} onOpenChange={setImportAbsOpen} importTarget="file" onImport={handleImport} expectedColumns={['Manager ID', 'Date', 'Justified', 'Reason']} />
      <ExcelImportDialog open={importLateOpen} onOpenChange={setImportLateOpen} importTarget="file" onImport={handleImport} expectedColumns={['Manager ID', 'Date', 'Period', 'Justified', 'Reason']} />
    </div>
  );
}
