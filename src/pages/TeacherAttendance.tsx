...
import { teacherAttendanceApi } from '@/services/attendance-api';
...
export default function TeacherAttendance() {
  ...
  const [bulkRows, setBulkRows] = useState<{ teacherId: string; sessionId: string; date: string; isJustified: boolean; reason?: string; period?: number }[]>([{ teacherId: '', sessionId: '', date: today(), isJustified: false, period: 10 }]);

  const absFilter: AttendanceFilter = { ...filter, page: absPage, limit: PAGE_LIMIT };
  const lateFilter: AttendanceFilter = { ...filter, page: latePage, limit: PAGE_LIMIT };

  const { data: absencesRes, isLoading: absLoading } = useQuery({ queryKey: ['teacher-absences', absFilter], queryFn: () => teacherAttendanceApi.getAbsences(absFilter) });
  const { data: latesRes, isLoading: lateLoading } = useQuery({ queryKey: ['teacher-lates', lateFilter], queryFn: () => teacherAttendanceApi.getLates(lateFilter) });
  ...
  const absBulk = useBulkSelection(absences);
  const lateBulk = useBulkSelection(lates);
  const bulkAbsStatusMut = useMutation({ mutationFn: ({ ids, status }: { ids: string[]; status: any }) => teacherAttendanceApi.bulkUpdateAbsenceStatus(ids, status), onSuccess: () => { invalidate(); absBulk.clear(); toast.success(t('common.statusUpdated')); } });
  const bulkAbsDeleteMut = useMutation({ mutationFn: (ids: string[]) => teacherAttendanceApi.bulkDeleteAbsences(ids), onSuccess: () => { invalidate(); absBulk.clear(); toast.success(t('common.bulkDeleted')); } });
  const bulkLateStatusMut = useMutation({ mutationFn: ({ ids, status }: { ids: string[]; status: any }) => teacherAttendanceApi.bulkUpdateLateStatus(ids, status), onSuccess: () => { invalidate(); lateBulk.clear(); toast.success(t('common.statusUpdated')); } });
  const bulkLateDeleteMut = useMutation({ mutationFn: (ids: string[]) => teacherAttendanceApi.bulkDeleteLates(ids), onSuccess: () => { invalidate(); lateBulk.clear(); toast.success(t('common.bulkDeleted')); } });
  ...
  const getTeacherName = (a: TeacherAbsence | TeacherLate) =>
    a.firstName ? `${a.firstName} ${a.lastName ?? ''}`.trim() : a.teacherId;

  const resetAbsForm = (a?: TeacherAbsence) => { setFormTeacherId(a?.teacherId || ''); setFormSessionId(a?.sessionId || ''); setFormDate(a?.date || today()); setFormJustified(a?.isJustified || false); setFormReason(a?.reason || ''); };
  const resetLateForm = (l?: TeacherLate) => { setFormTeacherId(l?.teacherId || ''); setFormSessionId(l?.sessionId || ''); setFormDate(l?.date || today()); setFormJustified(l?.isJustified || false); setFormReason(l?.reason || ''); setFormPeriod(l?.period || 10); };

  const handleAbsSubmit = () => {
    if (!formTeacherId) { toast.error(t('common.selectTeacher')); return; }
    const data = { teacherId: formTeacherId, sessionId: formSessionId, date: formDate, isJustified: formJustified, reason: formJustified ? formReason : undefined };
  ...
  const handleLateSubmit = () => {
    if (!formTeacherId) { toast.error(t('common.selectTeacher')); return; }
    const data = { teacherId: formTeacherId, sessionId: formSessionId, date: formDate, isJustified: formJustified, reason: formJustified ? formReason : undefined, period: formPeriod };
  ...
  const handleScanBulkAbs = (ids: string[]) => { setBulkRows(ids.map(id => ({ teacherId: id, sessionId: '', date: today(), isJustified: false }))); setBulkAbsDialog(true); };
  const handleScanBulkLate = (ids: string[]) => { setBulkRows(ids.map(id => ({ teacherId: id, sessionId: '', date: today(), isJustified: false, period: 10 }))); setBulkLateDialog(true); };

  const handleImport = async (file: File) => {
    const res = await teacherAttendanceApi.importExcel(file);
    if (res.success) { invalidate(); toast.success(`${res.data?.imported ?? 0} records imported`); }
    else toast.error(res.message);
  };

  const addBulkRow = () => setBulkRows(prev => [...prev, { teacherId: '', sessionId: '', date: today(), isJustified: false, period: 10 }]);
  ...
  const generateBulkRows = () => {
    ...
    setBulkRows(Array.from({ length: count }, () => ({ teacherId: '', sessionId: '', date: today(), isJustified: false, period: 10 })));
  };

  const handleFilterChange = (update: Partial<AttendanceFilter>) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <FilterBar showClear={!!(filter.entityId || filter.dateFrom || filter.dateTo || filter.levelId || filter.classId || filter.subjectId)} onClear={() => { setFilter({}); setAbsPage(1); setLatePage(1); }}>
        {activeView !== 'calendar' && (
          <>
            <div className="space-y-1 flex flex-col"><Label className="text-xs">{t('common.dateFrom')}</Label><DatePickerField value={filter.dateFrom || ''} onChange={v => handleFilterChange({ dateFrom: v || undefined })} placeholder={t('common.dateFrom')} className="w-40" /></div>
            <div className="space-y-1 flex flex-col"><Label className="text-xs">{t('common.dateTo')}</Label><DatePickerField value={filter.dateTo || ''} onChange={v => handleFilterChange({ dateTo: v || undefined })} placeholder={t('common.dateTo')} className="w-40" /></div>
          </>
        )}
        <div className="space-y-1">
          <Label className="text-xs">{t('common.teacher')}</Label>
          <TeacherSelect
            value={filter.entityId || 'all'}
            onChange={v => handleFilterChange({ entityId: v === 'all' ? undefined : v })}
            levelId={filter.levelId}
            classId={filter.classId}
            subjectId={filter.subjectId}
            allOption={{ value: 'all', label: t('common.allTeachers') }}
            className="w-44"
          />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">{t('common.class')}</Label>
          <ClassSelect value={filter.classId || 'all'} onChange={v => handleFilterChange({ classId: v === 'all' ? undefined : v })} levelId={filter.levelId} allOption={{ value: 'all', label: t('common.allClasses') }} className="w-36" />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">{t('common.level')}</Label>
          <LevelSelect value={filter.levelId || 'all'} onChange={v => handleFilterChange({ levelId: v === 'all' ? undefined : v })} allOption={{ value: 'all', label: t('common.allLevels') }} className="w-36" />
        </div>
        <div className="space-y-1">
          <Label className="text-xs">{t('common.subject')}</Label>
          <SubjectSelect value={filter.subjectId || 'all'} onChange={v => handleFilterChange({ subjectId: v === 'all' ? undefined : v })} allOption={{ value: 'all', label: t('common.allSubjects') }} className="w-36" />
        </div>
      ...
      <Tabs value={tab} onValueChange={v => setTab(v as any)}>
        <TabsList>
          <TabsTrigger value="absences" className="gap-2"><UserX className="h-4 w-4" />{t('attendance.absences')} ({absencesRes?.total ?? 0})</TabsTrigger>
          <TabsTrigger value="lates" className="gap-2"><Clock className="h-4 w-4" />{t('attendance.lates')} ({latesRes?.total ?? 0})</TabsTrigger>
        ...
        <TabsContent value="absences" className="space-y-4">
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <Button size="sm" variant="outline" onClick={() => { setBulkRows([{ teacherId: '', sessionId: '', date: today(), isJustified: false }]); setBulkAbsDialog(true); }}><ListPlus className="me-2 h-4 w-4" />{t('common.bulkAdd')}</Button>
            ...
            <Button size="sm" variant="outline" onClick={() => teacherAttendanceApi.exportExcel(filter)}><Download className="me-2 h-4 w-4" />{t('common.export')}</Button>
          </div>
          <BulkActionBar selectedCount={absBulk.count} onApprove={() => bulkAbsStatusMut.mutate({ ids: [...absBulk.selectedIds], status: 'APPROVED' })} onRemoveApproval={() => bulkAbsStatusMut.mutate({ ids: [...absBulk.selectedIds], status: 'PENDING' })} onDelete={() => bulkAbsDeleteMut.mutate([...absBulk.selectedIds])} />
          {absLoading ? <Skeleton className="h-48 w-full" /> : absView === 'calendar' ? (
            <AttendanceCalendarView items={absences} type="absences" getEntityName={(item) => getTeacherName(item as TeacherAbsence)} onEdit={(item) => { resetAbsForm(item as any); setEditingAbsence(item as any); }} onDelete={(item) => setDeleteTarget({ id: item.id, type: 'absence' })} />
          ) : (
            <div className="space-y-3">
              <div className="rounded-md border overflow-auto">
                <Table>
                  <TableHeader><TableRow>
                    <TableHead className="w-10"><Checkbox checked={absBulk.isAllSelected} onCheckedChange={() => absBulk.toggleAll()} /></TableHead>
                    <TableHead>{t('common.teacher')}</TableHead><TableHead>{t('common.session')}</TableHead><TableHead>{t('common.date')}</TableHead><TableHead>{t('common.status')}</TableHead><TableHead>{t('common.justified')}</TableHead><TableHead>{t('common.reason')}</TableHead><TableHead className="w-24">{t('common.actions')}</TableHead>
                  </TableRow></TableHeader>
                  <TableBody>
                    {absences.length === 0 ? <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground py-8">{t('attendance.noAbsences')}</TableCell></TableRow> :
                      absences.map(a => (
                        <TableRow key={a.id}>
                          <TableCell><Checkbox checked={absBulk.selectedIds.has(a.id)} onCheckedChange={() => absBulk.toggle(a.id)} /></TableCell>
                          <TableCell>{getTeacherName(a)}</TableCell>
                          <TableCell>{a.sessionName ?? a.sessionId}</TableCell>
              ...
              <Pagination page={absPage} totalPages={absencesRes?.totalPages ?? 1} total={absencesRes?.total ?? 0} onPageChange={setAbsPage} disabled={absLoading} />
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="lates" className="space-y-4">
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <Button size="sm" variant="outline" onClick={() => { setBulkRows([{ teacherId: '', sessionId: '', date: today(), isJustified: false, period: 10 }]); setBulkLateDialog(true); }}><ListPlus className="me-2 h-4 w-4" />{t('common.bulkAdd')}</Button>
          ...
          <BulkActionBar selectedCount={lateBulk.count} onApprove={() => bulkLateStatusMut.mutate({ ids: [...lateBulk.selectedIds], status: 'APPROVED' })} onRemoveApproval={() => bulkLateStatusMut.mutate({ ids: [...lateBulk.selectedIds], status: 'PENDING' })} onDelete={() => bulkLateDeleteMut.mutate([...lateBulk.selectedIds])} />
          {lateLoading ? <Skeleton className="h-48 w-full" /> : lateView === 'calendar' ? (
            <AttendanceCalendarView items={lates} type="lates" getEntityName={(item) => getTeacherName(item as TeacherLate)} onEdit={(item) => { resetLateForm(item as any); setEditingLate(item as any); }} onDelete={(item) => setDeleteTarget({ id: item.id, type: 'late' })} />
          ) : (
            <div className="space-y-3">
              <div className="rounded-md border overflow-auto">
                <Table>
                  <TableHeader><TableRow>
                    <TableHead className="w-10"><Checkbox checked={lateBulk.isAllSelected} onCheckedChange={() => lateBulk.toggleAll()} /></TableHead>
                    <TableHead>{t('common.teacher')}</TableHead><TableHead>{t('common.session')}</TableHead><TableHead>{t('common.date')}</TableHead><TableHead>{t('common.period')}</TableHead><TableHead>{t('common.status')}</TableHead><TableHead>{t('common.justified')}</TableHead><TableHead>{t('common.reason')}</TableHead><TableHead className="w-24">{t('common.actions')}</TableHead>
                  </TableRow></TableHeader>
                  <TableBody>
                    {lates.length === 0 ? <TableRow><TableCell colSpan={9} className="text-center text-muted-foreground py-8">{t('attendance.noLates')}</TableCell></TableRow> :
                      lates.map(l => (
                        <TableRow key={l.id}>
                          <TableCell><Checkbox checked={lateBulk.selectedIds.has(l.id)} onCheckedChange={() => lateBulk.toggle(l.id)} /></TableCell>
                          <TableCell>{getTeacherName(l)}</TableCell>
                          <TableCell>{l.sessionName ?? l.sessionId}</TableCell>
                          <TableCell>{l.date}</TableCell>
                          <TableCell>{l.period} {t('attendance.min')}</TableCell>
                          <TableCell><ApprovalStatusBadge status={l.status} /></TableCell>
              ...
              <Pagination page={latePage} totalPages={latesRes?.totalPages ?? 1} total={latesRes?.total ?? 0} onPageChange={setLatePage} disabled={lateLoading} />
            </div>
          )}
        </TabsContent>
      ...
      <Dialog open={absDialog || !!editingAbsence} onOpenChange={o => { if (!o) { setAbsDialog(false); setEditingAbsence(null); } }}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2"><Label>{t('common.teacher')}</Label><TeacherSelect value={formTeacherId} onChange={setFormTeacherId} requiredResult={formTeacherId ? [formTeacherId] : []} className="w-full" /></div>
            <div className="space-y-2"><Label>{t('common.session')}</Label><SessionSelect value={formSessionId} onChange={setFormSessionId} className="w-full" /></div>
      ...
      <Dialog open={bulkAbsDialog} onOpenChange={setBulkAbsDialog}>
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            {bulkRows.map((row, idx) => (
              <div key={idx} className="flex gap-2 items-end flex-wrap border-b border-border pb-2">
                <div className="flex-1 min-w-[160px] space-y-1"><Label className="text-xs">{t('common.teacher')}</Label><TeacherSelect value={row.teacherId} onChange={v => updateBulkRow(idx, 'teacherId', v)} requiredResult={row.teacherId ? [row.teacherId] : []} disabledIds={bulkRows.filter((_, i) => i !== idx).map(r => r.teacherId).filter(Boolean)} className="w-full" /></div>
                <div className="w-44 space-y-1"><Label className="text-xs">{t('common.session')}</Label><SessionSelect value={row.sessionId} onChange={v => updateBulkRow(idx, 'sessionId', v)} className="w-full" /></div>
          ...
          <DialogFooter><Button variant="outline" onClick={() => setBulkAbsDialog(false)}>{t('common.cancel')}</Button><Button onClick={() => { const valid = bulkRows.filter(r => r.teacherId); if (!valid.length) { toast.error(t('attendance.addValidRow')); return; } const ids = valid.map(r => r.teacherId); if (new Set(ids).size !== ids.length) { toast.error(t('attendance.duplicateEntity')); return; } bulkAbsMut.mutate(valid.map(r => ({ teacherId: r.teacherId, sessionId: r.sessionId, date: bulkShared ? bulkSharedDate : r.date, isJustified: r.isJustified, reason: r.isJustified ? r.reason : undefined }))); }}>{t('attendance.addCountAbsences', { count: bulkRows.filter(r => r.teacherId).length })}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={bulkLateDialog} onOpenChange={setBulkLateDialog}>
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          ...
          <DialogFooter><Button variant="outline" onClick={() => setBulkLateDialog(false)}>{t('common.cancel')}</Button><Button onClick={() => { const valid = bulkRows.filter(r => r.teacherId); if (!valid.length) { toast.error(t('attendance.addValidRow')); return; } const ids = valid.map(r => r.teacherId); if (new Set(ids).size !== ids.length) { toast.error(t('attendance.duplicateEntity')); return; } bulkLateMut.mutate(valid.map(r => ({ teacherId: r.teacherId, sessionId: r.sessionId, date: bulkShared ? bulkSharedDate : r.date, isJustified: r.isJustified, reason: r.isJustified ? r.reason : undefined, period: r.period ?? 10 }))); }}>{t('attendance.addCountLates', { count: bulkRows.filter(r => r.teacherId).length })}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <ExcelImportDialog open={importAbsOpen} onOpenChange={setImportAbsOpen} importTarget="file" onImport={handleImport} expectedColumns={['Teacher ID', 'Session ID', 'Date', 'Justified', 'Reason']} />
      <ExcelImportDialog open={importLateOpen} onOpenChange={setImportLateOpen} importTarget="file" onImport={handleImport} expectedColumns={['Teacher ID', 'Session ID', 'Date', 'Period', 'Justified', 'Reason']} />
    </div>
  );
}
